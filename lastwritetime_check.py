...
def event_handler(message, severity, event_class='/Cegeka/Eventlog', eventKey="", component="LastWriteTime check"):
    return {
...
def run_command(host, username, password, dcip, service, command):
    ...
    if service:
        command_to_run.extend(['--service', service])
    ...
    try:
        ...
        if errors:
            ...
            if exit_code != 0:
                ...
                return str(' '.join(stdout)).replace('The local farm is not accessible. Cmdlets with FeatureDependencyId are not registered.', '').strip()
...
def main():
    ...
    parser.add_argument('--service', dest='service', default='', help='http/https/wsman')
    args = parser.parse_args()

    try:
        host, username, password, dcip, service = args.remote, args.username, args.password, args.dcip, args.service
        file = args.file
        ...
        current_time = run_command(host, username, password, dcip, service, "Get-Date -UFormat '%d/%m/%Y %H:%M:%S'")
 ...
 select -first 1); Write-Host $file.lastwritetime.ToString('dd/MM/yyyy HH:mm:ss')".format(file)
        last_file_write_time = run_command(host, username, password, dcip, service, last_file_command)
        if last_file_write_time:
            last_file_write_time = last_file_write_time.replace('-', '/').replace('.', '/')
